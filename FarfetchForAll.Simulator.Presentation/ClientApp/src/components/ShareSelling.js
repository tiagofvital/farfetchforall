...
export default function ShareSelling() {
    const { register, handleSubmit } = useForm();

    const onSubmit = data => {
        var request = Adapter.transformSell(data);

        Controller.sell(request, (err, value) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <div class="form-group">
                <label for="inputShareSellingAmount">Amount: </label>
                <input id="inputShareSellingAmount" class="form-control" type="number" placeholder="1000" name="amount" ref={register({ required: true, maxLength: 80 })} />
            </div>
            <div class="form-group">
